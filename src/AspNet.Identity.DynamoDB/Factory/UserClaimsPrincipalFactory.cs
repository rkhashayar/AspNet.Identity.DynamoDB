...
public class UserClaimsPrincipalFactory<TUser>
    : IUserClaimsPrincipalFactory<TUser> where TUser : class
{
        ...
        IOptions<IdentityOptions> optionsAccessor)
    {
        ...
        if (optionsAccessor?.Value == null) throw new ArgumentNullException(nameof(optionsAccessor));
    ...
    public virtual async Task<ClaimsPrincipal> CreateAsync(TUser user)
    {
        ...
        if (UserManager.SupportsUserRole)
        {
            ...
            foreach (var roleName in roles) id.AddClaim(new Claim(Options.ClaimsIdentity.RoleClaimType, roleName));
        }
